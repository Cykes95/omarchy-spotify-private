...
function responseError(status, payload, fallback) {
  ...
  if (!message) message = fallback || "Spotify no pudo completar esta solicitud"
...
function rateLimitSuffix(retryAfter) {
  ...
  return ". Inténtalo de nuevo dentro de " + seconds + (seconds === 1 ? " segundo" : " segundos") + "."
}

function rateLimitMessage(retryAfter) {
  var suffix = rateLimitSuffix(retryAfter)
  return suffix ? "Spotify está ocupado" + suffix
    : "Spotify está ocupado. Inténtalo de nuevo en un momento."
}

// Spotify's development-mode quota is low enough that starting two library
// Three parallel reads remove the long single-file queue seen while opening
// the client. SpotifyApi immediately falls back to one after a 429 and honors
// Retry-After, so this remains conservative when Spotify pushes back.
var API_MAX_IN_FLIGHT = 3
var API_MIN_REQUEST_INTERVAL_MS = 1250
var API_MAX_RATE_LIMIT_RETRIES = 4

function apiMinimumRequestIntervalMs() {
  return API_MIN_REQUEST_INTERVAL_MS
}
...
function apiJobPriority(job) {
  ...
  if (String(job.priority || "") === "interactive") return 1
  return String(job.priority || "") === "background" ? -1 : 0
}
...
function playlistItemsHiddenMessage() {
  return "Spotify no muestra el contenido de esta lista salvo que seas su propietario o colaborador. Aun así puedes reproducirla como contexto de Spotify."
}
...
function playlistItemsEmptyMessage(playlist, itemCount, error, status, userId) {
  ...
  if (error) {
    ...
    return "No se pudo cargar esta lista. Inténtalo de nuevo en un momento."
  }
  if (count > 0) return ""
  if (playlistItemsHiddenByApi(200, owned, collaborative, knownUser))
    return playlistItemsHiddenMessage()
  return "Esta lista no tiene elementos visibles."
}

function localSocketFallbackMessage() {
  return "El reproductor local no estaba listo; esta canción se iniciará mediante Spotify."
}
...
function repeatModeLabel(mode) {
  ...
  if (value === "track") return "Esta canción"
  if (value === "context") return "Todo"
  return "Desactivado"
}

var TYPE_LABELS = {
  track: { singular: "Canción", plural: "Canciones" },
  artist: { singular: "Artista", plural: "Artistas" },
  album: { singular: "Álbum", plural: "Álbumes" },
  playlist: { singular: "Lista", plural: "Listas" },
  show: { singular: "Pódcast", plural: "Pódcasts" },
  episode: { singular: "Episodio", plural: "Episodios" },
  audiobook: { singular: "Audiolibro", plural: "Audiolibros" }
}

function typeLabel(type, plural) {
  var entry = TYPE_LABELS[String(type || "")]
  if (!entry) return plural ? "resultados" : "Elemento de Spotify"
...
var CATALOG_CACHE_LIMIT = 600000
var CATALOG_CACHE_DETAIL_LIMIT = 8
var CATALOG_CACHE_DETAIL_ITEM_LIMIT = 50

function catalogCacheRecord(value) {
  var source = value && typeof value === "object" && !Array.isArray(value)
    ? value : ({})
  function rows(key) {
    return arrayValues(source[key]).slice(0, 30)
  }
  var playlistRows = rows("playlists")
  var savedTrackRows = rows("savedTracks")
  var savedAlbumRows = rows("savedAlbums")
  var recentRows = rows("recentTracks")
  var topTrackRows = rows("topTracks")
  var topArtistRows = rows("topArtists")
  // Version 1 of this file predated explicit readiness flags. A non-empty
  // collection in that legacy record is still safe to show immediately.
  var legacy = source.playlistsLoaded === undefined
    && source.savedTracksLoaded === undefined && source.homeLoaded === undefined
  var detailSource = source.playlistDetails
  var albumSource = source.albumDetails
  var playlistDetails = []
  var albumDetails = []
  if (detailSource && typeof detailSource === "object") {
    var detailKeys = Object.keys(detailSource).slice(0, CATALOG_CACHE_DETAIL_LIMIT)
    for (var d = 0; d < detailKeys.length; d++) {
      var detail = detailSource[detailKeys[d]]
      if (!detail || typeof detail !== "object") continue
      var detailItem = detail.item
      if (!detailItem || detailItem.type !== "playlist" || !detailItem.id) continue
      playlistDetails.push({
        item: detailItem,
        items: arrayValues(detail.items).slice(0, CATALOG_CACHE_DETAIL_ITEM_LIMIT)
      })
    }
  }
  if (albumSource && typeof albumSource === "object") {
    var albumKeys = Object.keys(albumSource).slice(0, CATALOG_CACHE_DETAIL_LIMIT)
    for (var a = 0; a < albumKeys.length; a++) {
      var albumDetail = albumSource[albumKeys[a]]
      if (!albumDetail || !albumDetail.item || albumDetail.item.type !== "album"
          || !albumDetail.item.id) continue
      albumDetails.push({ item: albumDetail.item,
  ...
  var lastContextItems = []
  if (Array.isArray(source.lastContextItems)) {
    for (var ci = 0; ci < source.lastContextItems.length && ci < 50; ci++) {
      var cit = source.lastContextItems[ci]
      if (!cit || !cit.id) continue
      lastContextItems.push({
        kind: "item",
        type: String(cit.type || "track"),
        id: String(cit.id || ""),
        uri: String(cit.uri || ("spotify:track:" + cit.id)),
        name: String(cit.name || "Sin título"),
        subtitle: String(cit.subtitle || cit.artist || ""),
        album: String(cit.album || ""),
        artists: arrayValues(cit.artists),
        imageUrl: String(cit.imageUrl || cit.artUrl || ""),
        durationMs: Math.max(0, Number(cit.durationMs) || 0),
        externalUrl: String(cit.externalUrl || "")
      })
    }
  }
  ...
  var record = {
    ...
    playlistsLoaded: source.playlistsLoaded === true || (legacy && playlistRows.length > 0),
    savedTracksLoaded: source.savedTracksLoaded === true || (legacy && savedTrackRows.length > 0),
    savedAlbumsLoaded: source.savedAlbumsLoaded === true || (legacy && savedAlbumRows.length > 0),
    homeLoaded: source.homeLoaded === true || (legacy
      && (recentRows.length > 0 || topTrackRows.length > 0 || topArtistRows.length > 0)),
    playlists: playlistRows,
    savedTracks: savedTrackRows,
    savedAlbums: savedAlbumRows,
    recentTracks: recentRows,
    ...
    lastContextItems: lastContextItems
  }
  return JSON.stringify(record).length <= CATALOG_CACHE_LIMIT ? record : ({})
}

function parseCatalogCache(raw) {
  return catalogCacheRecord(parseJson(String(raw || ""), ({})))
}

function encodeCatalogCache(value) {
  return JSON.stringify(catalogCacheRecord(value), null, 2) + "\n"
}
...
function searchScope(tab, detailItem, selectedPlaylist, homeType, libraryType) {
  ...
  if (area === "detail" && detailItem) {
    ...
    label = homeLabels[selectedHome] || "Para ti"
    ...
    var libraryLabels = {
      tracks: "Canciones que te gustan",
      albums: "Álbumes guardados",
      artists: "Artistas seguidos",
      shows: "Pódcasts guardados",
      episodes: "Episodios guardados",
      audiobooks: "Audiolibros guardados"
    }
    var selectedLibrary = String(libraryType || "tracks")
    label = libraryLabels[selectedLibrary] || "Tu biblioteca"
...
function normalizeTrack(value, imageWidth, parentContext) {
  ...
  var subtitle = type === "episode"
    ? String((item.show && item.show.name) || item.description || "Pódcast")
    : (type === "chapter"
      ? String((item.audiobook && item.audiobook.name) || item.description || "Audiolibro")
  ...
  return {
    ...
    id: String(item.id || ""),
    uri: String(item.uri || ""),
    name: String(item.name || "Sin título"),
    subtitle: subtitle,
...
function normalizeContext(value, imageWidth) {
  ...
  if (type === "album") {
  ...
  else if (type === "playlist") subtitle = String((item.owner && item.owner.display_name) || "Lista")
  else if (type === "artist") subtitle = "Artista"
  else if (type === "show") subtitle = String(item.publisher || "Pódcast")
  else {
    var authors = []
    ...
    subtitle = authors.length ? authors.join(", ") : String(item.publisher || "Audiolibro")
  }
...
function visibleOrderPlaybackMessage(itemCount) {
  ...
  return count > PLAYBACK_URI_LIMIT
    ? "Reproduciendo el orden mostrado para 100 elementos; Spotify limita la reproducción personalizada a 100"
    : "Reproduciendo el orden mostrado"
}
