...
Item {
  ...
  function saveSettings(showStatus) {
    ...
    syncDraftSettings()
    if (showStatus !== false) service.succeed("Ajustes guardados")
  }
  ...
  function audioQualityLabel() {
    if (draftAudioQuality === "96 kbps") return "Estándar · 96 kbps"
    if (draftAudioQuality === "320 kbps") return "Muy alta · 320 kbps"
    return "Alta · 160 kbps"
  }

  function shortcutPlayerLabel(value) {
    if (value === "Full player") return "Reproductor completo"
    if (value === "Mini player") return "Minirreproductor"
    return "Aplicación Música de Omarchy"
  }
  ...
  function maxBarTextWidthLabel() {
    ...
    return value === 0 ? "Sin límite" : Math.round(value) + " px"
  }
  ...
  function connectionButtonText() {
    if (!service) return "Spotify no está disponible"
    if (service.loginBusy) return service.loginProgress + "…"
    if (fullyConnected) return "Conectado"
    if (accountConnected && !service.daemon.playbackReady)
      return "Configurar reproducción"
    if (accountConnected) return "Terminar de configurar la reproducción"
    if (!service.daemon.playbackReady) return "Configurar y continuar"
    return "Continuar con Spotify"
  }

  function connectionHeadline() {
    if (!service) return "Spotify no está disponible"
    if (service.daemon.setupBusy) return "Configurando la reproducción"
    if (fullyConnected) return "Ya está conectado"
    if (accountConnected) return "Cuenta conectada"
    if (!service.daemon.playbackReady) return "Una configuración rápida y listo"
    return "Continuar con Spotify"
  }

  function connectionErrorText() {
    if (!service) return "Omarchy Spotify no está disponible"
    return service.lastError || service.auth.lastError || service.daemon.lastError
  }

  function playbackStatusText() {
    if (!service) return "La reproducción no está disponible"
    if (!service.daemon.requirementsChecked) return "Comprobando la reproducción…"
    if (service.daemon.setupBusy) return "Preparando la reproducción en este equipo…"
    if (!service.daemon.playbackReady) return "Hace falta una configuración inicial"
    if (!service.daemon.credentialsChecked) return "Comprobando la conexión con Spotify…"
    if (!service.daemon.credentialsAvailable) return "Listo para iniciar sesión en Spotify"
    if (service.daemon.running) return "Activo en este equipo"
    return "Listo: se iniciará al reproducir música"
  }
  ...
  function openItem(item) {
    ...
    // Clear the previous detail before switching pages. Otherwise QML can
    // render one frame of the prior album/list and make a slow request look
    // as if the click was ignored.
  ...
  function openSidebarAlbum(item) {
    openItem(item)
    // The sidebar lives inside a nested ListView. Keep this final assignment
    // explicit: a detail request must never leave the visible page on Library.
    if (item && item.kind === "context") currentTab = "detail"
  }
  ...
  function playSelectedPlaylist() {
    ...
    if (collection && collection.playbackUsesVisibleOrder) {
      ...
      if (!items.length) {
        service.fail("No hay elementos visibles de la lista para reproducir")
        return
      }
  ...
  function activatePanelCursor() {
    ...
    else if (action === "shutdown") shutdownAndClose()
  ...
  function shortcutRows() {
    return [
      { section: "BÚSQUEDA", action: "Enfocar la búsqueda", keys: "Ctrl+F o /" },
      { action: "Alternar esta zona / todo Spotify", keys: "Ctrl+F o /" },
      { action: "Salir de la búsqueda", keys: "Esc" },
      { section: "NAVEGACIÓN", action: "Volver", keys: "Alt+Izquierda" },
      { action: "Salir de Ajustes o Dispositivos", keys: "Esc" },
      { action: "Abrir Ajustes", keys: "Ctrl+," },
      { action: "Abrir Para ti", keys: "Alt+Mayús+H" },
      { action: "Abrir Cola", keys: "Alt+Mayús+Q" },
      { action: "Abrir Dispositivos", keys: "Alt+Mayús+D" },
      { action: "Abrir el artista actual", keys: "Ctrl+Mayús+A" },
      { action: "Abrir el álbum actual", keys: "Ctrl+Mayús+B" },
      { action: "Moverse entre la barra lateral, búsqueda, lista y reproductor", keys: "Tab / F6" },
      { action: "Moverse a un control", keys: "Flechas" },
      { action: "Activar el control resaltado", keys: "Intro" },
      { action: "Acciones de fila", keys: "C" },
      { action: "Elegir una acción de fila", keys: "Flechas o Intro" },
      { action: "Moverse por las listas", keys: "Flechas" },
      { action: "Abrir el elemento seleccionado", keys: "Intro" },
      { section: "REPRODUCCIÓN", action: "Reproducir o pausar", keys: "Espacio" },
      { action: "Pista anterior", keys: "Ctrl+Izquierda" },
      { action: "Pista siguiente", keys: "Ctrl+Derecha" },
      { action: "Abrir letras en Omasing", keys: "Ctrl+Mayús+L" },
      { action: "Silenciar o restaurar volumen", keys: "M" },
      { action: "Alternar aleatorio", keys: "Ctrl+S" },
      { action: "Cambiar repetición", keys: "Ctrl+R" },
      { action: "Retroceder 10 segundos", keys: "Mayús+Izquierda" },
      { action: "Avanzar 10 segundos", keys: "Mayús+Derecha" },
      { action: "Subir volumen 5%", keys: "Ctrl+Arriba" },
      { action: "Bajar volumen 5%", keys: "Ctrl+Abajo" },
      { section: "VENTANA", action: "Preparar cierre / cerrar", keys: "Esc, Esc" },
      { action: "Ocultar pistas de atajos visibles", keys: "Ctrl+H" },
      { action: "Mostrar esta referencia", keys: "Ctrl+/" }
    ]
  }
  ...
  function shutdownAndClose() {
    // Mark this surface closed before stopping the on-demand daemon so its
    // visible-receiver helper cannot immediately start it again.
    close()
    if (service) service.stopEngine()
    if (shell && typeof shell.hide === "function") shell.hide(pluginId)
  }
  ...
  function primaryNavigationItems() {
    var items = [
      { id: "home", label: "Para ti", icon: "󰎆" },
      { id: "discover", label: "Descubrir", icon: "󰲸" }
    ]
    if (service && service.lastRadioPlaylist) items.push({
      id: "radio",
      label: service.lastRadioPlaying ? "Radio actual" : "Última radio",
      icon: "󰎆"
    })
    items.push({ id: "queue", label: "Cola", icon: "󰐕" })
    return items
  }
  ...
  function pageTitle() {
    if (currentTab === "login") return "Iniciar sesión en Spotify"
    if (showingUniversalSearch) return "Buscar en Spotify"
    if (currentTab === "home") return "Para ti"
    if (currentTab === "discover") return "Descubrir"
    if (currentTab === "library") return "Tu biblioteca"
    if (currentTab === "playlists") return "Listas"
    if (currentTab === "queue") return "Cola"
    if (currentTab === "devices") return "Spotify Connect"
    if (currentTab === "setup") return "Ajustes"
    if (currentTab === "detail") {
      if (artistScopedSearchActive) return "Buscar en " + service.detailItem.name
      return service && service.detailItem ? service.detailItem.name : "Cargando…"
    }
    return "Buscar"
  }

  function pageSubtitle() {
    if (currentTab === "login") return "Conecta tu cuenta de Spotify para empezar"
    if (showingUniversalSearch) return activeSearchScope.available
      ? "Buscando en todas partes: activa el filtro de zona para acotar los resultados"
      : "Canciones, artistas, álbumes, listas, pódcasts y audiolibros"
    if (currentTab === "home") return "Reproducido recientemente y tus favoritos"
    if (currentTab === "discover") return "Mezclas personales y música nueva de Spotify"
    if (currentTab === "library") return "Canciones, álbumes, artistas, pódcasts y audiolibros"
    if (currentTab === "playlists") return "Tus listas de Spotify"
    if (currentTab === "queue") return "Lo que sonará a continuación"
    if (currentTab === "devices") return "Altavoces y reproductores"
    if (currentTab === "setup") return "Cuenta, reproducción y preferencias de la aplicación"
    if (currentTab === "detail") {
      if (artistScopedSearchActive)
        return "Canciones, álbumes y listas que coinciden con «" + artistSearchText.trim() + "»"
      return service && service.detailItem
        ? Api.spotifyTypeLabel(service.detailItem.type) : "Elemento de Spotify"
    }
    return "Canciones, artistas, álbumes, listas, pódcasts y audiolibros"
  }
    ...
    contentItem: Column {
      ...
      Row {
        ...
        Column {
          ...
          Text {
            width: parent.width
            text: "El primer atajo ilumina los controles relacionados. Los atajos de reproducción se pausan mientras escribes."
            color: root.muted
            font.family: root.fontFamily
        ...
        Button {
          ...
          iconText: "󰅖"
          foreground: root.foreground
          tooltipText: root.shortcutHint("Cerrar", "Esc")
          focusable: true
  ...
  Popup {
    ...
    contentItem: FocusScope {
      ...
      Text {
        ...
        text: root.contextItem ? String(root.contextItem.name || "Elemento de Spotify") : "Elemento de Spotify"
        color: root.muted
        font.family: root.fontFamily
        font.pixelSize: Style.font.caption
        font.bold: true
      ...
      Text {
        ...
        leftPadding: Style.space(8)
        rightPadding: Style.space(8)
        text: "Flechas o Intro para elegir · Esc para cerrar"
        color: root.muted
        font.family: root.fontFamily
        font.pixelSize: Style.font.caption
      }
      ...
      ContextMenuButton {
        ...
        text: "Iniciar radio de la canción"
        iconText: "󰎆"
        onClicked: {
          mediaContextMenu.close()
      ...
      ContextMenuButton {
        ...
        text: root.service && root.service.playlistConversionBusy
          ? "Creando tu copia…" : "Convertir en tu propia lista"
        iconText: "󰒍"
      ...
      ContextMenuButton {
        ...
        text: root.service && root.service.isSaved(root.contextItem)
          ? "Quitar de la biblioteca" : "Guardar en la biblioteca"
    ...
    contentItem: Column {
      ...
      Text {
        ...
        text: root.pendingPlaylistItem
          ? "Añadir “" + String(root.pendingPlaylistItem.name || "canción") + "” a una lista"
          : "Añadir a una lista"
        color: root.foreground
        font.family: root.fontFamily
      ...
      Text {
        width: parent.width
        text: "Elige una de tus listas o crea una nueva lista privada."
        color: root.muted
        font.family: root.fontFamily
        font.pixelSize: Style.font.bodySmall
      ...
      Row {
        ...
        TextField {
          ...
          foreground: root.foreground
          placeholderText: "Nombre de la nueva lista"
  ...
  Popup {
    ...
    contentItem: Column {
      ...
      Text {
        width: parent.width
        text: "Pon un nombre a tu nueva lista privada."
        color: root.muted
        font.family: root.fontFamily
        font.pixelSize: Style.font.bodySmall
        wrapMode: Text.WordWrap
      ...
      TextField {
        ...
        width: parent.width
        foreground: root.foreground
        placeholderText: "Nombre de la lista"
  ...
  Popup {
    ...
    contentItem: Column {
      ...
      Button {
        width: parent.width
        text: "Después de este álbum o lista"
        iconText: "󰓛"
        foreground: root.foreground
        leftAlign: true
        ...
        Row {
          ...
          BorderSurface {
            ...
            Column {
              ...
              Button {
                width: parent.width
                text: root.compactWidth ? "" : "Canciones que te gustan"
                iconText: "󰋑"
                foreground: root.foreground
                ...
                tooltipText: "Canciones que te gustan"
              ...
              Row {
                ...
                Button {
                  ...
                  tooltipText: "Crear una lista nueva"
            ...
            Item {
              ...
              ListView {
                ...
                anchors.left: parent.left
                anchors.right: parent.right
                anchors.top: sidebarPlaylistsHeading.bottom
                anchors.topMargin: Style.space(4)
                height: Math.max(0, (parent.height - sidebarPlaylistsHeading.implicitHeight
                  - sidebarAlbumsHeading.implicitHeight - Style.space(16)) * 0.55)
              ...
              Text {
                id: sidebarAlbumsHeading
                anchors.left: parent.left
                anchors.right: parent.right
                anchors.top: playlistShortcuts.bottom
              ...
              ListView {
                ...
                anchors.left: parent.left
                anchors.right: parent.right
                anchors.top: sidebarAlbumsHeading.bottom
                anchors.bottom: parent.bottom
                anchors.topMargin: Style.space(4)
                model: root.service ? root.service.sidebarSavedAlbums() : []
                clip: true
                spacing: Style.space(1)
                reuseItems: true
                keyNavigationEnabled: false
                ...
                FastScrollHandler {
                  parent: albumShortcuts
                  flickable: albumShortcuts
                  onScrolled: {
                    if (albumShortcuts.atYEnd && root.service
                        && root.service.savedAlbumsNext
                        && !root.service.savedAlbumsLoading)
                      root.service.loadSavedAlbums(true)
                  }
                }
                ...
                delegate: Button {
                  required property var modelData
                  width: ListView.view.width
                  text: modelData.name || "Álbum"
                  iconText: "󰝚"
                  foreground: root.foreground
                  leftAlign: true
                  focusable: false
                  selected: root.service && root.service.detailItem
                    && root.service.detailItem.id === modelData.id
                  tooltipText: modelData.name || "Álbum"
                  onClicked: root.openSidebarAlbum(modelData)
                }
              }
            }
            ...
            Button {
              ...
              anchors.bottom: parent.bottom
              anchors.margins: Style.space(8)
              text: root.compactWidth ? "" : "Ajustes"
              ...
              tooltipText: root.shortcutHint("Ajustes", "Ctrl+,")
            ...
            Row {
              ...
              Column {
                width: Math.max(80, parent.width
                  ...
                  - shortcutHelpButton.width - refreshButton.width - shutdownButton.width
                  - closeButton.width
                  - parent.spacing * (shortcutHintsDismissButton.visible ? 5 : 4))
                anchors.verticalCenter: parent.verticalCenter
                spacing: Style.space(1)
              ...
              Button {
                ...
                anchors.verticalCenter: parent.verticalCenter
                text: "Ctrl+H · Ocultar pistas"
                foreground: root.foreground
                fontSize: Style.font.caption
                focusable: false
                tooltipText: "Ocultar las pistas hasta activarlas de nuevo en Ajustes · Ctrl+H"
              ...
              Button {
                ...
                tooltipText: "Apagar Spotify por completo"
                focusable: false
                hasCursor: root.cursorOn("header", "shutdown")
                onClicked: root.shutdownAndClose()
                onHovered: function(on) { if (on) root.setPanelCursor("header", "shutdown") }
              }
            ...
            Row {
              ...
              TextField {
                ...
                placeholderText: root.activeSearchScope.available && root.searchInContext
                  ? "Buscar en " + root.activeSearchScope.label : "Buscar en Spotify"
                ...
                PanelToolTip {
                  ...
                  text: root.activeSearchScope.available
                    ? "Buscar · Ctrl+F o /\nPulsa otra vez para alternar esta zona y todo Spotify"
                    : "Buscar · Ctrl+F o /"
                }
              ...
              Button {
                ...
                tooltipText: root.searchInContext
                  ? root.shortcutHint("Buscar en todo Spotify", "Ctrl+F o /")
                  : root.shortcutHint("Buscar solo en "
                    + root.activeSearchScope.label, "Ctrl+F o /")
              ...
              Column {
                ...
                Row {
                  ...
                  Text {
                    ...
                    text: root.service && root.service.title
                      ? root.service.title : "No se está reproduciendo nada"
                    color: root.foreground
                    font.family: root.fontFamily
                    font.pixelSize: Style.font.body
                    font.bold: true
                  ...
                  Row {
                    ...
                    Button {
                      ...
                      horizontalPadding: Style.space(4)
                      verticalPadding: Style.space(2)
                      tooltipText: root.shortcutHint("Acciones de la canción", "C")
                ...
                CursorSurface {
                  ...
                  PanelToolTip {
                    ...
                    text: root.shortcutHint("Abrir artista", "Ctrl+Mayús+A")
                  }
                  ...
                  ArtistLinks {
                    ...
                    fallbackText: root.service && root.service.artist
                      ? root.service.artist : "Elige algo para reproducir"
                ...
                CursorSurface {
                  ...
                  PanelToolTip {
                    ...
                    text: root.shortcutHint("Abrir álbum", "Ctrl+Mayús+B")
                  }
            ...
            Column {
              ...
              Row {
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint("Aleatorio", "Ctrl+S")
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint("Anterior", "Ctrl+Izquierda")
                  enabled: root.service && root.service.playbackControllable
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint(
                    root.service && root.service.playing ? "Pausar" : "Reproducir", "Espacio")
                  enabled: root.service && root.service.playbackControllable
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint("Siguiente", "Ctrl+Derecha")
                  enabled: root.service && root.service.playbackControllable
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint("Repetir: "
                ...
                TransportButton {
                  ...
                  tooltipText: root.shortcutHint("Abrir letras en Omasing",
                    "Ctrl+Shift+L")
              ...
              Row {
                ...
                PlaybackSlider {
                  ...
                  PanelToolTip {
                    ...
                    text: "Avanzar o retroceder 10 segundos · Mayús+Izquierda / Mayús+Derecha"
                  }
                  KeyHint {
                    region: "footer"
                    action: "seek"
            ...
            Column {
              ...
              Row {
                ...
                Button {
                  ...
                  tooltipText: root.shortcutHint("Dispositivos", "Alt+Mayús+D")
                ...
                Button {
                  ...
                  tooltipText: root.service ? root.service.sleepStatusText() : "Temporizador de apagado"
                ...
                PlaybackSlider {
                  ...
                  PanelToolTip {
                    ...
                    text: "Volumen · Ctrl+Arriba / Ctrl+Abajo · M para silenciar"
                  }
                  KeyHint {
                    region: "footer"
                    action: "volume"
              ...
              Text {
                ...
                text: root.service
                  ? ((root.service.playing ? "Reproduciendo en " : "Conectado a ")
    ...
    Item {
      Column {
        ...
        Row {
          ...
          Repeater {
            model: [
              { type: "recent", label: "Reproducido recientemente", icon: "󰋚" },
              { type: "tracks", label: "Canciones más escuchadas", icon: "󰎈" },
              { type: "artists", label: "Artistas más escuchados", icon: "󰠃" }
            ]
            Button {
              required property var modelData
        ...
        MediaCollection {
          ...
          emptyMessage: root.service && root.service.homeLoading
            ? "Cargando tu historial de escucha…"
            : (root.homeFilter.trim() ? "No hay coincidencias en " + root.activeSearchScope.label + "."
    ...
    Item {
      MediaCollection {
        ...
        emptyMessage: root.service && root.service.discoverLoading
          ? "Buscando listas seleccionadas para ti…"
          : (root.discoverFilter.trim() ? "No matches in Discover."
            : (root.service && root.service.discoverMessage
            ? root.service.discoverMessage : "Aún no hay listas de descubrimiento disponibles."))
        onActivated: function(item, items, uri) {
          root.activateMedia(item, items, uri)
        }
        onOpened: function(item) { root.openItem(item) }
      ...
      Column {
          ...
          Row {
            ...
            Column {
              ...
              Text {
                ...
                text: root.service && root.service.detailItem
                  ? root.service.detailItem.name : "Cargando…"
                color: root.foreground
                font.family: root.fontFamily
                font.pixelSize: Style.font.title
                font.bold: true
            ...
            Column {
              ...
              Button {
                ...
                iconText: "󰇙"
                foreground: root.foreground
                tooltipText: root.shortcutHint("Más acciones", "C")
                focusable: false
          ...
          Row {
            ...
            Column {
              ...
              Text {
                ...
                width: parent.width
                text: root.artistSearchText.trim() ? "ÁLBUMES Y EP" : "ÁLBUMES Y EP DESTACADOS"
                color: root.foreground
                font.family: root.fontFamily
                font.pixelSize: Style.font.caption
                font.bold: true
              ...
              MediaCollection {
                ...
                emptyMessage: root.service && (root.service.artistAlbumsLoading
                  ...
                  ? "Buscando lanzamientos…" : "No hay álbumes ni EP coincidentes."
                onActivated: function(item, items, uri) {
                  root.activateMedia(item, items, uri)
                }
                onOpened: function(item) { root.openItem(item) }
            ...
            Column {
              ...
              Text {
                ...
                width: parent.width
                text: root.artistSearchText.trim() ? "CANCIONES" : "LAS 10 CANCIONES DESTACADAS"
                color: root.foreground
                font.family: root.fontFamily
                font.pixelSize: Style.font.caption
                font.bold: true
              ...
              MediaCollection {
                ...
                emptyMessage: root.service && (root.service.artistSongsLoading
                  || root.service.detailLoading)
                  ? "Buscando canciones…" : "No hay canciones coincidentes."
                onActivated: function(item, items, uri) {
                  root.activateMedia(item, items, uri)
                }
                onOpened: function(item) { root.openItem(item) }
          ...
          Column {
            ...
            Text {
              ...
              text: detailRoot.searchLoading
                ? "Buscando en " + (root.service && root.service.detailItem
                  ? root.service.detailItem.name : "este artista") + "…"
            ...
            Text {
              ...
              text: "No hay canciones, álbumes ni listas que coincidan con la búsqueda."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
        ...
        MediaCollection {
          ...
          emptyMessage: root.service && root.service.detailMessage
            ? root.service.detailMessage : "No hay elementos disponibles para esta selección."
          onActivated: function(item, items, uri) {
            root.activateMedia(item, items, uri)
          }
          onOpened: function(item) { root.openItem(item) }
    ...
    Item {
      ...
      Column {
        ...
        Row {
          ...
          Repeater {
            model: [
              { type: "track", label: "Canciones" },
              { type: "artist", label: "Artistas" },
              { type: "album", label: "Álbumes" },
              { type: "playlist", label: "Listas" },
        ...
        Column {
          ...
          Text {
            width: parent.width
            visible: !root.service || root.service.searchHistory.length === 0
            text: "Escribe un título, artista, álbum, lista, pódcast, episodio o audiolibro."
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
            wrapMode: Text.WordWrap
        ...
        MediaCollection {
          ...
          emptyMessage: root.service && root.service.searchLoading
            ? "Buscando…" : "No hay " + Api.searchTypeLabel(root.searchType)
              + " results."
          onActivated: function(item, items, uri) {
            root.activateMedia(item, items, uri)
          }
    ...
    Item {
      ...
      Column {
        ...
        Row {
          ...
          Repeater {
            model: [
              { type: "tracks", label: "Canciones", icon: "󰎈" },
              { type: "albums", label: "Álbumes", icon: "󰀥" },
              { type: "artists", label: "Artistas", icon: "󰠃" },
              { type: "shows", label: "Pódcasts", icon: "󰦔" },
              { type: "episodes", label: "Episodios", icon: "󰐾" },
              { type: "audiobooks", label: "Audiolibros", icon: "󰂺" }
            ]
            Button {
              required property var modelData
              text: modelData.label
        ...
        MediaCollection {
          ...
          emptyMessage: root.service && root.service.libraryLoading(root.libraryType)
            ...
            : (root.libraryFilter.trim()
              ? "No hay coincidencias en " + root.activeSearchScope.label + "."
              : "No hay elementos guardados en esta sección.")
          onActivated: function(item, items, uri) {
            root.activateMedia(item, items, uri)
          }
          onOpened: function(item) { root.openItem(item) }
      ...
      Column {
        ...
        Column {
          ...
          SearchableDropdown {
            ...
            accent: root.accent
            fontFamily: root.fontFamily
            placeholderText: "Elige una lista…"
            emptyText: root.service && root.service.playlistsLoading
              ? "Cargando listas…" : "No se encontraron listas"
          ...
          Button {
            ...
            text: root.service && root.service.playlistsLoading
              ? "Cargando más listas…" : "Cargar más listas"
            iconText: "󰑐"
            foreground: root.foreground
          ...
          Row {
            ...
            Text {
              ...
              text: root.service && root.service.selectedPlaylist
                ? root.service.selectedPlaylist.name : "Selecciona una lista"
              color: root.foreground
              font.family: root.fontFamily
              font.pixelSize: Style.font.subtitle
              font.bold: true
            ...
            Button {
              ...
              tooltipText: playlistItemsCollection.playbackUsesVisibleOrder
                ...
                : "Reproducir esta lista en su orden original"
          ...
          Button {
            ...
            selected: true
            enabled: root.service && !root.service.playlistActionBusy
            tooltipText: "Copiar todos los elementos disponibles y quitar después el original seguido"
        ...
        MediaCollection {
          ...
          emptyMessage: root.service && root.service.selectedPlaylist
            ? (root.service.playlistItemsEmptyMessage
              || "Esta lista no tiene elementos visibles.")
            : (root.compactWidth ? "Elige una lista arriba."
              : "Elige una lista en la barra lateral.")
    ...
    Item {
      ...
      Column {
        ...
        Row {
          ...
          Button {
            id: queueRefresh
              text: root.service && root.service.queueLoading ? "Cargando…" : "Actualizar"
            iconText: "󰑐"
            foreground: root.foreground
          ...
          Text {
            ...
            text: root.service && root.service.queueLoading
              ...
              : (root.queueFilter.trim()
                ? "No hay canciones coincidentes en la cola."
                : "No hay nada en cola para reproducirse después.")
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.body
            wrapMode: Text.WordWrap
        ...
        Column {
          ...
          Row {
            ...
            Text {
              ...
              text: "Elige dónde suena tu música. Aquí aparecen este equipo y los dispositivos cercanos de Spotify Connect."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              wrapMode: Text.WordWrap
            ...
            Button {
              id: deviceRefresh
              text: root.service && root.service.devicesLoading ? "Cargando…" : "Actualizar"
              iconText: "󰑐"
              foreground: root.foreground
          ...
          Text {
            ...
            text: "Buscando altavoces y este equipo…"
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
            wrapMode: Text.WordWrap
          ...
          Text {
            width: parent.width
            text: "Un altavoz cercano puede tardar unos instantes en conectarse la primera vez que lo eliges."
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
            wrapMode: Text.WordWrap
            ...
            Row {
              ...
              Column {
                ...
                Text {
                  ...
                  text: deviceRow.modelData.type
                    ...
                    + (deviceRow.modelData.local ? " · este equipo" : "")
                    + (deviceRow.modelData.localDiscovery ? " · cercano" : "")
                    + (deviceRow.modelData.restricted
                      ? (deviceRow.modelData.active
                        ? (root.service && root.service.sonosControlAvailable
                          ? " · controles locales" : " · controles limitados")
                        : " · no disponible") : "")
                  color: root.muted
                  font.family: root.fontFamily
                  font.pixelSize: Style.font.caption
              ...
              Text {
                ...
                text: root.service && root.service.deviceActivationBusy
                    && deviceRow.modelData.id === root.service.selectedDeviceId ? "Conectando"
          ...
          Text {
            ...
            visible: root.service && root.service.devicesLoaded
              && root.service.devices.length === 0
            text: "No hay dispositivos de Spotify Connect disponibles ahora. Comprueba que estén conectados y actualiza."
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.bodySmall
            wrapMode: Text.WordWrap
        ...
        Column {
          ...
          Column {
            ...
            Text {
              width: parent.width
              horizontalAlignment: Text.AlignHCenter
              text: "Tu música, biblioteca, listas y dispositivos de Spotify Connect, integrados en Omarchy."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              wrapMode: Text.WordWrap
            ...
            Column {
              ...
              Row {
                ...
                Column {
                  ...
                  Text {
                    text: root.service ? root.service.loginProgress : "Spotify no está disponible"
                    color: root.fullyConnected
              ...
              Text {
                width: parent.width
                text: "Dos pasos rápidos. Primero conecta tu cuenta de Spotify para explorar. Después autoriza la reproducción en este equipo si quieres escuchar aquí."
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.body
                wrapMode: Text.WordWrap
                  ...
                  Text {
                    text: root.service && root.service.auth.loggedIn
                      ...
                      : (root.service && root.service.auth.loginBusy
                        ...
                        : "Tu cuenta y biblioteca de Spotify")
                    color: root.foreground
                    font.family: root.fontFamily
                    font.pixelSize: Style.font.bodySmall
                  }
                ...
                Row {
                  ...
                  Text {
                    text: root.service && root.service.daemon.credentialsAvailable
                      ? "La reproducción en este equipo está conectada"
                      : (root.service && root.service.daemon.authenticationBusy
                        ? "Conectando la reproducción en este equipo…"
                        : "Reproducción en este equipo")
                    color: root.foreground
                    font.family: root.fontFamily
                    font.pixelSize: Style.font.bodySmall
                  }
              ...
              Text {
                ...
                text: !root.service || root.service.daemon.playbackReady
                  ...
                  : (root.service.daemon.binaryAvailable
                    ? "Esto prepara la reproducción privada y bajo demanda para tu cuenta."
                    : "Omarchy puede pedir la contraseña de tu equipo para instalar su pequeño componente de reproducción.")
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.caption
                wrapMode: Text.WordWrap
          ...
          Text {
            width: parent.width
            horizontalAlignment: Text.AlignHCenter
            text: "La contraseña se introduce solo en la página de Spotify. Omarchy Spotify nunca la ve."
            color: root.muted
            font.family: root.fontFamily
            font.pixelSize: Style.font.caption
            wrapMode: Text.WordWrap
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            Text {
              width: parent.width
              text: "Conecta Spotify para buscar, explorar tu biblioteca, gestionar listas y escuchar en este equipo u otro dispositivo de Spotify Connect."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              wrapMode: Text.WordWrap
            ...
            BorderSurface {
              ...
              Text {
                ...
                anchors.fill: parent
                anchors.margins: Style.space(8)
                text: !root.service ? "Spotify no está disponible"
                  : (root.service.loginBusy ? root.service.loginProgress + "…"
                  : (root.fullyConnected ? "Conectado y listo para reproducir"
                  : (root.service.auth.loggedIn
                    ? "Spotify está conectado · la reproducción necesita autorización"
                    : (root.service.daemon.credentialsAvailable
                      ? "La reproducción está lista · Spotify necesita autorización"
                      : "No conectado"))))
                color: root.fullyConnected ? root.accent : root.foreground
                font.family: root.fontFamily
                font.pixelSize: Style.font.body
            ...
            Row {
              ...
              Button {
                text: "Reconectar Spotify"
                iconText: "󰑐"
                foreground: root.foreground
                visible: root.service && root.service.auth.loggedIn
                enabled: root.service && !root.service.loginBusy
                tooltipText: "Reconecta si no funcionan la biblioteca o las listas"
          ...
          Column {
            ...
            Text {
              text: "REPRODUCCIÓN EN ESTE EQUIPO"
              color: root.foreground
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              font.bold: true
            ...
            Text {
              width: parent.width
              text: "Un reproductor ligero en segundo plano se inicia solo cuando lo necesitas, funciona con los controles multimedia de Omarchy y aparece en Spotify Connect como este equipo."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.body
              wrapMode: Text.WordWrap
          ...
          Column {
            ...
            Row {
              ...
              Column {
                ...
                Text {
                  text: "APAGAR POR INACTIVIDAD · MINUTOS"
                  color: root.muted
                  font.family: root.fontFamily
                  font.pixelSize: Style.font.caption
                  font.bold: true
            ...
            Column {
              ...
              Button {
                text: "Minirreproductor primero · "
                  + (root.draftShowMiniPlayer ? "On" : "Off")
                iconText: "󰍹"
                foreground: root.foreground
                selected: root.draftShowMiniPlayer
                tooltipText: root.draftShowMiniPlayer
                  ? "Al pulsar el icono de la barra se abre primero el minirreproductor"
                  : "Al pulsar el icono de la barra se abre directamente el reproductor completo"
                onClicked: {
            ...
            Column {
              ...
              Button {
                text: "Super + Mayús + M · " + root.shortcutPlayerLabel(root.draftShortcutPlayer)
                iconText: "󰌌"
                foreground: root.foreground
                selected: root.draftShortcutPlayer !== "Omarchy Music app"
                focusable: true
                tooltipText: "Alternar entre Música de Omarchy, reproductor completo y minirreproductor"
                onClicked: root.cycleShortcutPlayer()
              }

              Text {
                width: parent.width
                text: "Alterna entre Música de Omarchy → Reproductor completo → Minirreproductor. Super+Shift+M abre después esa opción."
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.bodySmall
                wrapMode: Text.WordWrap
              ...
              Button {
                text: "Pistas de atajos · "
                  + (root.draftShortcutHints ? "Activadas" : "Desactivadas")
                iconText: "󰘳"
                foreground: root.foreground
                selected: root.draftShortcutHints
                tooltipText: root.draftShortcutHints
                  ? "El primer atajo ilumina los controles coincidentes con la siguiente tecla"
                  : "Los atajos siguen funcionando sin la ayuda sobre los controles"
                onClicked: {
              ...
              Text {
                width: parent.width
                text: "Después de un atajo o Tab, los botones coincidentes se iluminan y muestran la siguiente tecla. Mantén Ctrl, Mayús o Alt para ver las combinaciones, o pulsa Ctrl+H para ocultarlas. Puedes activarlas aquí cuando quieras."
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.bodySmall
                wrapMode: Text.WordWrap
            ...
            Column {
              ...
              Flow {
                ...
                Button {
                  text: "Título · " + (root.draftShowTitle ? "Activado" : "Desactivado")
                  foreground: root.foreground
                  selected: root.draftShowTitle
                  tooltipText: "Mostrar el título de la canción en la barra superior"
                  onClicked: {
                ...
                Button {
                  text: "Artista · " + (root.draftShowArtist ? "Activado" : "Desactivado")
                  foreground: root.foreground
                  selected: root.draftShowArtist
                  tooltipText: "Mostrar el nombre del artista en la barra superior"
                  onClicked: {
                ...
                Button {
                  text: "En pausa · "
                    + (root.draftShowPausedTrack ? "Mostrar" : "Ocultar")
                  foreground: root.foreground
                  selected: root.draftShowPausedTrack
                  tooltipText: root.draftShowPausedTrack
                    ? "Mantener visibles el título y artista configurados durante la pausa"
                    : "Mostrar solo el icono de Spotify durante la pausa"
                  onClicked: {
                ...
                Button {
                  text: "Desplazar texto largo · " + (root.draftScrollBarText ? "Activado" : "Desactivado")
                  foreground: root.foreground
                  ...
                  tooltipText: root.barTextWidthUnlimited
                    ? "No disponible con ancho ilimitado: el texto siempre cabe"
                    : "Desplazar el texto solo cuando no cabe en la barra"
                  onClicked: {
                ...
                Button {
                  text: "Ancho · " + root.maxBarTextWidthLabel()
                  foreground: root.foreground
                  selected: root.barTextWidthExpanded
                  enabled: Api.canScrollBarText(root.draftShowTitle, root.draftShowArtist)
                  tooltipText: "Hasta qué ancho puede crecer el texto de la barra"
              ...
              Column {
                ...
                Text {
                  ...
                  text: "Los títulos muy largos ocuparán espacio de otros widgets de la barra."
                  color: root.muted
                  font.family: root.fontFamily
                  font.pixelSize: Style.font.bodySmall
                  wrapMode: Text.WordWrap
              ...
              Text {
                ...
                text: "Los textos largos se desplazan y se desvanecen en los bordes solo cuando superan el espacio disponible."
                color: root.muted
                font.family: root.fontFamily
                font.pixelSize: Style.font.bodySmall
                wrapMode: Text.WordWrap
            ...
            Column {
              ...
              Button {
                text: "Calidad de audio · " + root.audioQualityLabel()
                iconText: "󰎈"
                foreground: root.foreground
                tooltipText: "Cambiar la calidad de reproducción"
            ...
            Text {
              width: parent.width
              text: "Este equipo permanece visible en Spotify Connect mientras el reproductor está abierto. Al cerrarlo, un receptor vacío se apaga al vencer el tiempo de inactividad; el contenido pausado sigue disponible para reanudar. Usa 0 minutos para mantener disponible este equipo incluso con el reproductor cerrado. El nombre del dispositivo y la calidad de audio se aplican al iniciar la próxima reproducción local."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.bodySmall
              wrapMode: Text.WordWrap
            ...
            Text {
              width: parent.width
              text: "Los cambios se aplican inmediatamente. El nombre del dispositivo y la calidad de audio se actualizan al iniciar la próxima reproducción local."
              color: root.muted
              font.family: root.fontFamily
              font.pixelSize: Style.font.bodySmall
              wrapMode: Text.WordWrap
