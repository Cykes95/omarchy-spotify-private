...
runtime_session_dir=${XDG_RUNTIME_DIR:-"/run/user/$(id -u)"}/omarchy-spotify
start_marker="$runtime_session_dir/allow-start"
...
case $action in
  ...
  start)
    ...
    install -d -m 700 -- "$runtime_session_dir"
    : >"$start_marker"
    chmod 600 -- "$start_marker"
