...
Item {
  ...
  property double nextRequestAt: 0
  ...
  // Low-priority cache warmups are disposable. A real action must never wait
  // behind one while Spotify is responding slowly.
  property var backgroundRequests: []
  ...
  function markJobFinished(job) {
    ...
    forgetBackgroundRequest(job && job.handle)
    return true
  }

  function forgetBackgroundRequest(handle) {
    if (!handle) return
    var next = []
    for (var i = 0; i < backgroundRequests.length; i++)
      if (backgroundRequests[i] !== handle) next.push(backgroundRequests[i])
    backgroundRequests = next
  }

  function cancelBackgroundRequests() {
    var pending = backgroundRequests.slice()
    backgroundRequests = []
    for (var i = 0; i < pending.length; i++) abortRequest(pending[i])
  }
  ...
  function expireTimedOutRequests(timestamp) {
    ...
    for (var i = 0; i < jobs.length; i++) {
      ...
      deliverJob(job, 0, null,
        "Spotify tardó demasiado en responder. Inténtalo de nuevo.", null)
    }
  }
  ...
  function pumpRequests() {
    ...
    while (requestsInFlight < Api.apiInFlightLimit(restrictInFlight)) {
      var wait = Math.max(
        Api.apiCooldownMs(now(), rateLimitedUntil),
        Api.apiCooldownMs(now(), nextRequestAt))
      if (wait > 0) {
      ...
      requestsInFlight += 1
      nextRequestAt = now() + Api.apiMinimumRequestIntervalMs()
      startJob(taken.job)
    }
  ...
  function startJob(job) {
    ...
    if (!url) {
      finishJob(job, 0, null, "Algo salió mal al contactar con Spotify", null)
      return
    }
      ...
      try {
        ...
        xhr.onreadystatechange = function() {
          ...
          var error = ok ? "" : root.requestError(xhr.status, payload, xhr,
            "Spotify no pudo completar esta solicitud")
  ...
  function request(method, path, query, body, callback, options) {
    ...
    if (job.priority === "background")
      backgroundRequests = backgroundRequests.concat([handle])
    else if (Api.apiRequestIsMutating(method) || job.priority === "interactive")
